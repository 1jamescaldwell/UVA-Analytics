...
def change_log(file1_path, file2_path, result_path):
    ...
    for sheet in all_sheet_names:
        ...
        if df1 is not None and df2 is not None:
            ...
            if 'SOCSEC1' not in df2.columns or ('SOCSEC1' in df2.columns and df2['SOCSEC1'].duplicated().any()):
                ...
                # This only works if there are multiple rows for a socsec but they all have the same comments.  
                    # If there are multiple rows for the same student and there are different notes for each row, this will grab the first comment
                ...
                # print(sheet)
                # if sheet in ['ZKI004E08','BNB008W02','ASI006W02','BNB008W01','DFE010E02','BFE001W04','BNB001W03','BNB006W01','BNB006W05','BSI003W01','BFE001W04','BNB006W05','BSI003W01','BFE001E03']:
                ...
                df2.to_excel(writer, sheet_name=sheet, index=False,startrow=3) # Write the original DataFrame to the output file, since it still has the Notes columns
                ...
                if df1_sorted.equals(df2_sorted): # If files are identical
                    ...
                    df1.to_excel(writer, sheet_name=sheet, index=False,startrow=3) # Write the original DataFrame to the output file, since it still has the Notes columns
                    ...
                    output_df.to_excel(writer, sheet_name=sheet, index=False,startrow=3)
            ...
            ws.cell(row=2, column=2, value=xl2_meta[sheet].iloc[0, 1]) # Definition
            ws.cell(row=3, column=5, value=xl2_meta[sheet].iloc[1, 2]) # Link
            # ws.cell(row=2, column=2, value=xl2_meta[sheet].iloc[0, 2]) # Link
    ...
    print(warning_df.head(10))
    # Write summary
    pd.DataFrame({"Summary": summary}).to_excel(writer, sheet_name="Comparison Summary", index=False)
    # pd.DataFrame({"Warning": warning_df}).to_excel(writer, sheet_name="Comparison Summary", index=False)
    # print(summary)
...
if __name__ == "__main__":
    ...
    save_name = f'{file_name1[:-5]}_{file_name2[:-5]}_3.xlsx'
